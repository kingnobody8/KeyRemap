...
#ifdef SFML_SYSTEM_ANDROID /*HABIG EDIT*/
...
/*HABIG EDIT*/
#if SFML_OPENGL_ES_VER > 1 || !defined(SFML_OPENGL_ES)
...
#else
		sf::priv::ResourceStream rstream(filename);

		sf::Int64 res = rstream.seek(0);
		if (res == -1) //cannot open file
			return false;

		sf::Int64 pos = rstream.tell();
		if (pos < 0) //cannot seek file
			return false;

		sf::Int64 size = rstream.getSize();
		if (size == 0)
			return false; //empty file

		buffer.resize(static_cast<std::size_t>(size));
		rstream.read(&buffer[0], size);
		buffer.push_back('\0');
#endif
...
Shader::~Shader()
{
    ...
    if (m_shaderProgram) /*HABIG EDIT*/
        glCheck(GLEXT_glDeleteProgramObject(castToGlHandle(m_shaderProgram)));
}
...
{
#ifdef GLEXT_geometry_shader4 /*HABIG EDIT*/
    Lock lock(isAvailableMutex);
...
bool Shader::compile(const char* vertexShaderCode, const char* geometryShaderCode, const char* fragmentShaderCode)
{
    ...
    if (vertexShaderCode)
    {
        ...
        if (success == GL_FALSE)
        {
            ...
            glCheck(GLEXT_glDeleteProgramObject(shaderProgram));
            return false;
        }
    ...
    {
#ifdef GLEXT_GL_GEOMETRY_SHADER
        // Create and compile the shader
        ...
        // Check the compile log
        GLint success;
        glCheck(GLEXT_glGetShaderiv(geometryShader, GLEXT_GL_OBJECT_COMPILE_STATUS, &success));
        if (success == GL_FALSE)
        {
            char log[1024];
	...
	/*HABIG EDIT*/
    GLint success;
    glCheck(GLEXT_glGetProgramiv(shaderProgram, GLEXT_GL_OBJECT_LINK_STATUS, &success));
    if (success == GL_FALSE)
    {
        char log[1024];
...
/*HABIG EDIT*/
#else // SFML_OPENGL_ES_VERSION > 1 || !define(SFML_OPENGL_ES)
...
/*HABIG EDIT*/
#endif // SFML_OPENGL_ES_VERSION > 1 || !define(SFML_OPENGL_ES)
