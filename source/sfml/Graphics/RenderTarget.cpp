...
namespace
{
	...
	const char* vsShader =
		...
		"uniform mat4         u_TexMatrix;        // A constant representing the texture matrix.                                                \n"
		"                                                                                                                                       \n"
		...
		"    v_Color = a_Color;                                                                                                                    \n"
		"                                                                                                                                        \n"
		"    // Multiply the texture coordinates by the texture matrix to get the final normalized coords.                                        \n"
		"    v_TexCoordinate = vec4(u_TexMatrix * a_TexCoordinate).xy;                                                                            \n"
		"                                                                                                                                        \n"
		"    // gl_Position is a special variable used to store the final position.                                                                \n"
		"    // Multiply the vertex by the MVP matrix to get the final point in normalized screen coordinates.                                    \n"
	...
	const char* psShader =
		...
		"uniform bool        u_UseTexture;        // Is the texture valid.                                                                        \n"
		"                                                                                                                                        \n"
		"varying vec4        v_Color;            // This is the color from the vertex shader interpolated across the triangle per fragment.        \n"
		"varying vec2        v_TexCoordinate;    // This is the interpolated texture coordinate per fragment.                                    \n"
		"                                                                                                                                        \n"
		"void main()                                                                                                                            \n"
		"{                                                                                                                                        \n"
		"    if(u_UseTexture)                                                                                                                    \n"
...
{
#if defined(SFML_FIXED_FUNCTION)
    // Nothing to draw?
	...
	if (isActive(m_id) || setActive(true))
	{
		//non fixed function requires a shader
		if (states.shader == nullptr)
		{
			states.shader = &m_defaultShader;
		}
		...
		if (states.shader)
		{
			...
			{
				states.shader->setUniform("u_UseTexture", states.texture != NULL);
			}
		}

		//convert to expanded verticies
		if (m_vertexExpanded.size() < vertexCount)
			m_vertexExpanded.resize(vertexCount);
		for (int i = 0; i < vertexCount; ++i)
		{
			m_vertexExpanded[i] = VertexExp(vertices[i]);
		}

		GLint aPos = glGetAttribLocation(states.shader->getNativeHandle(), "a_Position");
		glCheck(aPos);
		GLint aClr = glGetAttribLocation(states.shader->getNativeHandle(), "a_Color");
		glCheck(aClr);
		GLint aTex = glGetAttribLocation(states.shader->getNativeHandle(), "a_TexCoordinate");
		glCheck(aTex);
		GLint uMat = glGetUniformLocation(states.shader->getNativeHandle(), "u_MVPMatrix");
		glCheck(uMat);
		...
		//glCheck(glBindTexture(GL_TEXTURE_2D, states.texture->getNativeHandle()));

		// Tell the texture uniform sampler to use this texture in the shader by binding to texture unit 0.
		//GLint uTex = glGetUniformLocation(states.shader->getNativeHandle(), "u_Texture");
		//glCheck(GLEXT_glUniform1i(uTex, 0));

		const sf::Transform mvpMatrix = m_view.getTransform() * states.transform;
		glCheck(glUniformMatrix4fv(uMat, 1, false, mvpMatrix.getMatrix()));
		glCheck(glVertexAttribPointer(aPos, 2, GL_FLOAT, GL_FALSE, sizeof(VertexExp), (char*)(&m_vertexExpanded[0]) + offsetof(VertexExp, position)));
		glCheck(glEnableVertexAttribArray(aPos));
		glCheck(glVertexAttribPointer(aClr, 4, GL_FLOAT, GL_FALSE, sizeof(VertexExp), (char*)(&m_vertexExpanded[0]) + offsetof(VertexExp, color)));
		glCheck(glEnableVertexAttribArray(aClr));
		glCheck(glVertexAttribPointer(aTex, 2, GL_FLOAT, GL_FALSE, sizeof(VertexExp), (char*)(&m_vertexExpanded[0]) + offsetof(VertexExp, texCoords)));
		glCheck(glEnableVertexAttribArray(aTex));
		...
		glCheck(glDisableVertexAttribArray(aPos));
		glCheck(glDisableVertexAttribArray(aClr));
		glCheck(glDisableVertexAttribArray(aTex));
	}
#endif
}
...
#if defined(SFML_FIXED_FUNCTION) /*HABIG EDIT*/
...
void RenderTarget::pushGLStates()
{
    if (isActive(m_id) || setActive(true))
    {
		...
		#ifndef SFML_OPENGL_ES /*HABIG EDIT*/
...
{
    if (isActive(m_id) || setActive(true)) /*HABIG EDIT*/
    {
		#if defined(SFML_FIXED_FUNCTION)
		    glCheck(glMatrixMode(GL_PROJECTION));
		    glCheck(glPopMatrix());
...
void RenderTarget::resetGLStates()
{
    ...
    if (isActive(m_id) || setActive(true))
    {
        ...
        {
#if defined(GLEXT_glClientActiveTexture) /*HABIG EDIT*/
...
void RenderTarget::initialize()
{
	...
	const bool result = m_defaultShader.loadFromMemory(vsShader, psShader);
	assert(result && "default shader compilation failed");

#endif //SFML_FIXED_FUNCTION
}
...
#else
	//HABIG TODO GLES
	// mvp is set inside draw instead
#endif
...
#else
	//hABIG TODO
	//transform is sent with MVP inside draw func
#endif
}


////////////////////////////////////////////////////////////
void RenderTarget::applyTexture(const Texture* texture, Shader* shader)
{
    Texture::bind(texture, Texture::Pixels, shader);
