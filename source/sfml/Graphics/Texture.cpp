...
void Texture::update(const Texture& texture, unsigned int x, unsigned int y)
{
	...
	//HABIG TODO support texture copy in open gl es > 1
...
////////////////////////////////////////////////////////////
/*HABIG EDIT*/
void Texture::bind(const Texture* texture, CoordinateType coordinateType, Shader* shader)
{
    TransientContextLock lock;
...
#else
			//bind the texture matrix to the fragment shader as a uniform.
			if (shader)
			{
				shader->setUniform("u_TexMatrix", *(Glsl::Mat4*)(matrix));
			}
#endif
        }
    }
