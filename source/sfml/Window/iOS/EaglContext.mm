...
/*HABIG EDIT*/
#if SFML_OPENGL_ES_VER == 3
const EAGLRenderingAPI ContextVersion = kEAGLRenderingAPIOpenGLES3;
#elif SFML_OPENGL_ES_VER == 2
const EAGLRenderingAPI ContextVersion = kEAGLRenderingAPIOpenGLES2;
#else
const EAGLRenderingAPI ContextVersion = kEAGLRenderingAPIOpenGLES1;
#endif
namespace
{
...
m_clock       ()
{
	...
	/*HABIG EDIT*/
    if (shared)
        m_context = [[EAGLContext alloc] initWithAPI:ContextVersion sharegroup:[shared->m_context sharegroup]];
    else
        m_context = [[EAGLContext alloc] initWithAPI:ContextVersion];
}
...
GlFunctionPointer EaglContext::getFunction(const char* name)
{
	...
	/*HABIG EDIT TODO this is only checkin GLES v1, needs to also handle v2 and v3.*/
    if (!module)
